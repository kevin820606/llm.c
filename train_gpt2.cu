...
__global__ void encoder_forward_kernel2(float *out, int *inp, float *wte,
                                        float *wpe, int B, int T, int C) {
...
__global__ void mean_kernel(float *mean, float *inp, int N, int C,
                            int block_size) {
...
__global__ void rstd_kernel(float *rstd, float *inp, float *mean, int N, int C,
                            int block_size) {
  extern __shared__ float shared[];
  int idx = blockIdx.x;  // range [0, B*T)
  int tid = threadIdx.x; // range [0, block_size)
...
__global__ void normalization_kernel(float *out, float *inp, float *mean,
                                     float *rstd, float *weight, float *bias,
                                     int B, int T, int C) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void crossentropy_forward_kernel1(float *losses, float *probs,
...
void encoder_forward(float *out, int *inp, float *wte, float *wpe, int B, int T,
  ...
  encoder_forward_kernel2<<<grid_size, block_size>>>(out, inp, wte, wpe, B, T,
                                                     C);
...
void matmul_forward(float *out, float *inp, float *weight, float *bias, int B,
  ...
  stat = cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, OC, B * T, C, &alpha,
                     weight, C, inp, C, &beta, out, OC);
...
void attention_forward(float *out, float *vaccum, float *qkvr, float *preatt,
  ...
  const float alpha = 1.0f;
  const float beta = 0.0f;
  stat = cublasSgemmStridedBatched(handle, CUBLAS_OP_T, CUBLAS_OP_N, T, T, HS,
                                   &alpha, k, HS, T * HS, q, HS, T * HS, &beta,
                                   preatt, T, T * T, B * NH);
  if (stat != CUBLAS_STATUS_SUCCESS) {
    printf("cublasSgemm failed\n");
    exit(1);
  ...
  softmax_forward_kernel4<<<grid_size, softmax_block_size, shared_mem_size>>>(
      att, preatt, B * NH * T, T);

  // new approach: first cuBLAS another batched matmul
  // y = att @ v # (B, nh, T, T) @ (B, nh, T, hs) -> (B, nh, T, hs)
  stat = cublasSgemmStridedBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N, HS, T, T,
                                   &alpha, v, HS, T * HS, att, T, T * T, &beta,
                                   vaccum, HS, T * HS, B * NH);
  if (stat != CUBLAS_STATUS_SUCCESS) {
    printf("cublasSgemm failed\n");
    exit(1);
...
void softmax_forward(float *out, float *inp, int N, int C) {
  ...
  softmax_forward_kernel4<<<grid_size, block_size, shared_mem_size>>>(out, inp,
                                                                      N, C);
}
...
void crossentropy_forward(float *losses, float *probs, int *targets, int B,
  ...
  const int N = B * T;
  const int grid_size = CEIL_DIV(N, block_size);
  crossentropy_forward_kernel1<<<grid_size, block_size>>>(losses, probs,
                                                          targets, B, T, V);
  cudaCheck(cudaGetLastError());
}
...
float *malloc_and_point_activations(ActivationTensors *acts,
  ...
  float **ptrs[] = {
      ...
      &acts->logits,    &acts->probs,     &acts->losses,   &acts->qkvr,
      &acts->v_accum};
...
void gpt2_forward(GPT2 *model, int *inputs, int *targets, int B, int T) {
  ...
  if (targets != NULL) {
    ...
    cudaCheck(cudaMemcpy(cpu_losses, acts.losses, B * T * sizeof(float),
  ...
  for (int step = 0; step <= 40; step++) {
    ...
    if (step > 0 && step % 20 == 0) {
      ...
      for (int t = 1; t < gen_max_length; t++) {
        ...
        cudaCheck(cudaMemcpy(cpu_probs, probs,
